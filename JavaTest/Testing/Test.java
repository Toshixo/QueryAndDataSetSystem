...
import JavaExperienceTestPackage.FileManagement;
import JavaExperienceTestPackage.QuerySystemUI;
import JavaExperienceTestPackage.SystemQueries;
...
class Test {
	...
	void testGettingHighestAgedPerson() {
		...
		JSONArray OP = Query.getOldestPerson(QuerySystem.personArray);
...
/**
 * broken Test + Not needed however kept for reference
 */
	@org.junit.jupiter.api.Test
...
//		assertEquals(value, "query screen");
	...
	void testGetPersonBasedOnFirstName() {
		QuerySystem.setValues();
		QuerySystem.parseJSONFromFile("t.ndjson");
		String value = Query.getPeopleBasedOnFirstLetter('D', QuerySystem.personArray).length() + "";
